...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const delegationsPageData = useQuery(api.manager.getDelegationsPageData, isSignedIn ? {} : "skip");
  ...
  }, [authLoaded, clerkUser, ensureCurrentUser, isSignedIn, userLoaded]);
  ...
  const loading = !authLoaded ||
    !userLoaded ||
    syncing ||
    (isSignedIn && (currentUser === undefined || delegationsPageData === undefined));
  ...
  const session = useMemo<AuthSession | null>(() => (user ? { userId: user.id } : null), [user]);
