...
export const updateEmployee = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const role of Array.from(new Set(nextRoles))) {
      await ctx.db.insert("userRoles", {
    ...
    await recordAudit(ctx, {
      ...
      changedBy: identity.subject,
      oldData: profile,
      newData: { ...profile, departmentId: args.departmentId, managerUserId: args.managerId },
    });
...
export const bulkInsertBadgeMappings = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const mapping of args.mappings) {
      ...
      if (!existing) {
        insertedIds.push(
          await ctx.db.insert("badgeMappings", {
...
export const toggleBiometricsConfig = mutation({
  ...
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const config = await ctx.db.get(args.configId);
    await ctx.db.patch(args.configId, { isActive: !config?.isActive, updatedAt: now() });
    return { ok: true };
  },
});
...
export const testBiometricsConnection = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (config.vendor === "generic_webhook") {
      const convexSiteUrl = getEnv("CONVEX_SITE_URL") ?? getEnv("VITE_CONVEX_SITE_URL") ?? "your-convex-site-url";
...
export const ingestBiometricsWebhook = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (config.webhookSecret && config.webhookSecret !== args.webhookSecret) {
      return {
        success: false,
        status: 401,
